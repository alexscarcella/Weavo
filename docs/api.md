...
| `MP.ganttView`, `MP.ganttRow`, `MP.ganttCell`, `MP.cellPopover`, `MP.cellSelection`, `MP.cellShiftSelection`, `MP.legend` | `js/ui/gantt/*.js` | The main grid view — `MP.cellSelection` (click/shift-click) drives the bulk-allocation popover, `MP.cellShiftSelection` (Ctrl+click) is a separate, independent range selection feeding only the shift feature's right-click menu, so the two never share state |
