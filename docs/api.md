...
| `MP.modal`, `MP.toast`, `MP.contextMenu`, `MP.toolbar`, `MP.datasetHeader` | `js/ui/common/*.js` | Generic UI building blocks shared across views — `MP.datasetHeader.renderDatasetHeader` is the info-line + color-legend header shared by the gantt and resource-load pages; `MP.modal.promptProjectForm`/`showProjectCard` are the project-team edit form and read-only info card; `MP.toolbar` also hosts the "+ New project", "💾 Backup", "Backup on exit" toggle (auto-runs the backup on tab/window close, best-effort — see `docs/deployment.md`), and "Change data folder…" menu actions (the last one resets `state` to `not-connected`, releasing `dirHandle`/`dataset` so the user can pick a different folder without a page reload) |
