...
| `MP.milestones` | `js/model/milestones.js` | `computeBaselineMilestones` (per-baseline effective release week, feeds the Milestones page), `countUpcomingBaselines` (release week ≥ today, feeds the "upcoming baselines" count in `MP.datasetHeader`), `computeUpcomingMilestonesByMonth` (upcoming releases only, grouped by calendar month, feeds the Milestones page's copyable list) |
| `MP.weekShift` | `js/model/week-shift.js` | `canShiftWeeks`/`shiftWeeksData` — one week/range of one task shifted one week back or forward, feeds the gantt view's shift feature; blocked if the task or any selected week is completed. `canShiftBaseline`/`shiftBaselineData` — a whole baseline (every non-completed task, milestones included) shifted by an arbitrary signed number of weeks in one go, feeds `MP.baselineCrud.shiftBaseline`; individually completed weeks stay fixed at their original position instead of moving, and the operation is blocked if that would collide with a moving week |
