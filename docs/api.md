...
| `MP.legacyMigration` | `js/data/legacy-migration.js` | `migrateIfNeeded(dirHandle)` — one-time upgrade of a pre-existing data folder still on an older `schemaVersion`; pure transform functions (`transformManifest`, `transformTeamResources`, `transformProject`). See [database.md](database.md#legacy-data-migration) |
...
| `MP.overallocation` | `js/model/overallocation.js` | `buildAllocationIndex`, `findAllocations`, `findOverallocatedKeys` — cross-project initials×week index; skips completed tasks and individually completed weeks |
| `MP.validation` | `js/model/validation.js` | `findOrphanTeam`, `findOrphanResources`, `findTeamMismatches`, `findOrphanProjectReferents` — non-blocking data-consistency warnings; `findTeamMismatches` skips completed tasks and individually completed weeks |
| `MP.milestones` | `js/model/milestones.js` | `computeBaselineMilestones` (per-baseline effective release week for EACH of the 2 shared milestone types — `readyForUat`/`uat` — returned as independent sub-results per row; the 3rd type, `taskDeadline`, is per-task and stays out of this report; feeds the Milestones page), `countUpcomingBaselines` (returns `{ readyForUat, uat }`, release week ≥ today per type, feeds the "Upcoming: …" counts in `MP.datasetHeader`), `computeUpcomingMilestonesByMonth` (upcoming releases of both shared types, each row tagged with its `type`/`label`, grouped by calendar month, feeds the Milestones page's copyable list) |
| `MP.milestoneRules` | `js/model/milestone-rules.js` | `checkChange`/`checkTaskChange`/`checkBaselineChange` — hard-block validation that a task's 3 milestone dates (`taskDeadline`/`readyForUat`/`uat`) stay strictly increasing in that order, never the same week; `checkTaskOrderingAfterShift` — same check projected forward by a whole-baseline shift delta; `collectTaskMilestoneDates` — a task's 3 dates as `{taskDeadline, readyForUat, uat}`. Shared by every write path that can move a milestone week: `MP.cellPopover`/`gantt-view.js`, `MP.weekShift`, `MP.taskCrud.moveTaskToPosition` |
| `MP.weekShift` | `js/model/week-shift.js` | `canShiftWeeks`/`shiftWeeksData` — one week/range of one task shifted one week back or forward, feeds the gantt view's shift feature; blocked if the task or any selected week is completed, or if it would break the milestone ordering invariant (via `MP.milestoneRules`). `canShiftBaseline`/`shiftBaselineData` — a whole baseline (every non-completed task, milestones included) shifted by an arbitrary signed number of weeks in one go, feeds `MP.baselineCrud.shiftBaseline`; individually completed weeks stay fixed at their original position instead of moving, and the operation is blocked if that would collide with a moving week or break the milestone ordering invariant |
...
| `MP.ganttView`, `MP.ganttRow`, `MP.ganttCell`, `MP.cellPopover`, `MP.cellSelection`, `MP.taskDrag`, `MP.legend` | `js/ui/gantt/*.js` | The main grid view — `MP.cellSelection` (click/shift-click) is the single range-selection mechanism, highlighting only (no popup on click); right-click resolves it via `getRangeForAction` and drives both the allocation popover (`MP.cellPopover`, always) and the shift menu (only when the range already has an allocation) at once — see `MP.ganttView.openCellContextMenu`; double-click is disabled. `MP.taskDrag` drives dragging a task's row handle onto another row to reposition it (including across baselines of the same project, via `MP.taskCrud.moveTaskToPosition` — a cross-baseline move adopts the destination baseline's `readyForUat`/`uat` dates onto the task and validates via `MP.milestoneRules`, blocking the move with an alert if that would break the ordering invariant) — keeps fully separate module state from `MP.cellSelection` |
