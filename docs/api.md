...
| `MP.weekShift` | `js/model/week-shift.js` | `canShiftWeeks`/`shiftWeeksData` — one week/range of one task shifted one week back or forward, feeds the gantt view's shift feature; `canShiftBaseline`/`shiftBaselineData` — a whole baseline (every non-completed task, milestones included) shifted by an arbitrary signed number of weeks in one go, feeds `MP.baselineCrud.shiftBaseline` |
